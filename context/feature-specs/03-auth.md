...
Override Clerk appearnce variables using the app's existing CSS variables. Do not hardcode colors.
...
Wrap the root layout with `ClerkProvdider` using Clerk's `dark` theme.

Create sign-in and sign-up pages using Clerk componenets.
...
Keep Clerk's default user menu and profile flows intact. Do not rebuilt or heavily customize Clerk internals.
